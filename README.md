# Hi, I'm Tracy Van Holt, PhD 👋

**Systems Scientist · Applied AI Researcher · Spatial & Network Analyst**

I work where knowledge graphs end — in origin communities, smallholder networks, and first-mile supply chains where fragmented data, not production capacity, is what limits decisions. I bring three capabilities that trade flow data alone cannot provide: **spatial intelligence** linking supply chain nodes to land cover and environmental systems; **consensus analysis** measuring whether actors genuinely share standards or only appear to; and **human-in-the-loop AI evaluation** catching hallucinations and failure modes before they reach decisions.

---

## 🔬 What I Do

**Human-in-the-Loop AI Evaluation**
I identify where AI models break down — hallucinations, misrepresentations, misclassifications — and design structured frameworks to catch and correct them before outputs reach decisions. Applied across NLP pipelines, ESG datasets, supply-chain analytics, and spatial systems.

**Spatial Intelligence & First-Mile Systems**
I link supply chain actors to land cover change, ocean systems, and environmental data at scales from local fishing communities to global commodity flows. I know what first-mile data looks like versus what gets reported — and where that gap creates compliance and decision risk.

**Consensus & Standards Analysis**
I use cultural consensus methods to measure whether actors genuinely share beliefs, standards, or practices — or only appear to. Applied to sustainability certification, ESG alignment, and supply chain governance. Predicts where enforcement gaps will emerge before they do.

**Applied Research Leadership**
Founded and directed an interdisciplinary research network. Led programs at NYU Stern and the Stockholm Resilience Centre. Deep familiarity with compliance frameworks: EUDR, SASB, TNFD, SEC Reg S-K, UNDP SDGs. Secured ~$1M in competitive grants across NSF, NASA, RWJ Foundation, and others.

---

## 🛠️ Technical Skills

**Methods**

![Spatial Analysis](https://img.shields.io/badge/Spatial_Analysis-2E8B57?style=flat-square)
![Network Analysis](https://img.shields.io/badge/Network_Analysis-2E8B57?style=flat-square)
![NLP / LLM Pipelines](https://img.shields.io/badge/NLP_%2F_LLM_Pipelines-2E8B57?style=flat-square)
![Machine Learning](https://img.shields.io/badge/Machine_Learning-2E8B57?style=flat-square)
![Model Evaluation](https://img.shields.io/badge/Model_Evaluation_%26_Validation-2E8B57?style=flat-square)
![Human-in-the-Loop](https://img.shields.io/badge/Human--in--the--Loop_Design-2E8B57?style=flat-square)

**Tools & Platforms**

![ArcGIS](https://img.shields.io/badge/ArcGIS-2C7AC3?style=flat-square)
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=flat-square&logo=tableau&logoColor=white)
![UCINET](https://img.shields.io/badge/UCINET-2E8B57?style=flat-square)
![Atlas.ti](https://img.shields.io/badge/Atlas.ti-FF6B35?style=flat-square)
![SPSS](https://img.shields.io/badge/SPSS-052FAD?style=flat-square)
![RISE_360](https://img.shields.io/badge/RISE_360_%2F_Canvas-6A0DAD?style=flat-square)
![Claude](https://img.shields.io/badge/Claude-D4A017?style=flat-square)
![ChatGPT](https://img.shields.io/badge/ChatGPT-74AA9C?style=flat-square&logo=openai&logoColor=white)

---

## 📊 Research Impact

| Metric | Value |
|---|---|
| Peer-reviewed publications | 31+ |
| Total citations | ~4,000 |
| h-index | 22 |
| Papers with 100+ citations | 9 |
| Competitive grant funding | ~$1M |

---

## 📄 Selected Publications

*Selected work most relevant to AI/data strategy, systems evaluation, and supply chain analytics*

| Paper | Venue | Year |
|---|---|---|
| [Resilience in Crisis: Strengthening Food Supply Networks during the COVID-19 Pandemic](https://link.springer.com/book/9783032061249) *(Albrecht, Johnson, **Van Holt**, Carter, Perrett)* | Book Chapter — *The COVID Information Commons* (in press) | 2026 |
| [Does Sustainability Generate Better Financial Performance?](https://doi.org/10.1080/20430795.2022.2106934) *(Atz, **Van Holt**, Liu, Bruno)* | Journal of Sustainable Finance & Investment | 2023 |
| [Fishery Improvement Projects as a Governance Tool](https://doi.org/10.1371/journal.pone.0223054) *(Crona, Käll, **Van Holt**)* | PLoS One | 2019 |
| [Perpetuating the Myth of the Return of Native Forests](https://doi.org/10.1126/sciadv.1601768) *(**Van Holt** & Putz)* | Science Advances | 2017 |
| [A Social Wellbeing in Fisheries Tool (SWIFT)](VanHolt_2016_SWIFT_FisheriesWellbeing_Sustainability.pdf) *(**Van Holt**, Weisman et al.)* | Sustainability | 2016 |
| [Using Social–Ecological Syndromes to Understand Impacts of Seafood Trade](Crona_VanHolt_2015_SeafoodTrade_SmallScaleFisheries_GEC.pdf) *(Crona, **Van Holt** et al.)* | Global Environmental Change | 2015 |
| [Influence of Landscape Change on Nearshore Fisheries in Southern Chile](VanHolt_2012_LandscapeChange_NearshoreChile_GCB.pdf) *(**Van Holt**, Moreno et al.)* | Global Change Biology | 2012 |
| [Structure of Ethnic Violence in Sudan: Semi-Automated Network Analysis](VanHolt_2012_SudanViolence_NetworkAnalysis_CMOT.pdf) *(**Van Holt**, Johnson et al.)* | Computational & Mathematical Organization Theory | 2012 |
| [Assessing Local Knowledge of Game Abundance Using Consensus Analysis](https://doi.org/10.1007/s10745-010-9354-2) *(**Van Holt**, Townsend, Cronkleton)* | Human Ecology | 2010 |

*Full list on [Google Scholar](https://scholar.google.com/citations?user=Srk2uEMAAAAJ)*

---

## 🌍 Selected Work

**Supply Chain AI & Information Systems**
- Mapped 107 seafood supply chains across 50 stakeholder leaders — revealed that information asymmetry, not resource scarcity, drives coordination failure
- Analyzed Ceylon cinnamon supply chain in Sri Lanka — revealed fragmented information (not production capacity) limits market access
- Evaluated ESG outputs for Fortune 500 clients (Mars, McDonald's, Big-Box Retail, Automotive) — revealed sustainability-driven supply chain organization generates measurable financial returns

**AI Evaluation & NLP**
- Designed and benchmarked NLP pipelines for the Office of Naval Research: semi-automated methods achieved ~88% precision vs. ~38% for fully automated — quantifying the precision gap and the value of human-in-the-loop design
- Evaluated and iteratively refined AI outputs across supply-chain and analytical workflows; identified hallucinations and failure modes in production contexts
- Developed and validated ML classifications for remote sensing; diagnosed systematic model errors that propagate downstream

**Spatial & Network Modeling**
- Built multi-layer behavioral networks linking 106 fishers, 311 species, 11 communities, and 17 management areas — revealing cross-scale interactions between land, ocean, and human behavior
- Validated machine learning land classifications at ~200 km local and ~1,200 km regional scales — revealed that misclassifying tree plantations as native forest distorts environmental models

**Curriculum & AI Literacy**
- Designed AI-assisted curriculum for ASU: 5 modules, 20 skills, 4 datasets — demonstrated that non-experts can evaluate spatial AI outputs with structured frameworks

---

## 🎓 Background

**Ph.D. & M.S., Interdisciplinary Ecology** — University of Florida (2009)
*Fulbright Scholar · NASA Fellow · David Boren Fellow (DoD) · Rotary Scholar*

**B.S., Biochemistry** — SUNY Stony Brook

**Field Research:** Chile · Brazil · Bolivia · Indonesia · Madagascar · Sweden

**Off-Site Research (data & remote analysis):** Sri Lanka · Rwanda · Sudan & South Sudan

**Languages:** Spanish (fluent) · Portuguese & Swedish (conversational) · Mapudugun & Guarayo (working knowledge)

---

## 📂 Publication Archive

PDFs of peer-reviewed publications are organized as `Author_Year_ShortTitle_Journal.pdf` and available directly in this repository.

| File | Title | Journal | Year |
|---|---|---|---|
| [Crona_VanHolt_2015...](Crona_VanHolt_2015_SeafoodTrade_SmallScaleFisheries_GEC.pdf) | Using Social-Ecological Syndromes to Understand Impacts of Seafood Trade | *Global Environmental Change* | 2015 |
| [VanHolt_2012_LandscapeInfluences...](VanHolt_2012_LandscapeInfluences_FisherSuccess_EcolSoc.pdf) | Landscape Influences on Fisher Success | *Ecology & Society* | 2012 |
| [VanHolt_2012_LandscapeChange...](VanHolt_2012_LandscapeChange_NearshoreChile_GCB.pdf) | Influence of Landscape Change on Nearshore Fisheries in Southern Chile | *Global Change Biology* | 2012 |
| [VanHolt_2012_SudanViolence...](VanHolt_2012_SudanViolence_NetworkAnalysis_CMOT.pdf) | Structure of Ethnic Violence in Sudan: Semi-Automated Network Analysis | *Computational & Mathematical Organization Theory* | 2012 |
| [VanHolt_2013_RapidEthnographic...](VanHolt_2013_RapidEthnographic_CulturalMapping_Poetics.pdf) | Rapid Ethnographic Assessment for Cultural Mapping | *Poetics* | 2013 |
| [VanHolt_2016_ExpertEffect...](VanHolt_2016_ExpertEffect_CulturalModels_HDW.pdf) | Influence of the Expert Effect on Cultural Models | *Human Dimensions of Wildlife* | 2016 |
| [VanHolt_2016_TreePlantations...](VanHolt_2016_TreePlantations_ForestTransitions_LUP.pdf) | A Stand of Trees Does Not a Forest Make: Tree Plantations and Forest Transitions | *Land Use Policy* | 2016 |
| [VanHolt_Weisman_2016...](VanHolt_Weisman_2016_GlobalProductionNetworks_COES.pdf) | Global Production Network Mapping for Transforming Socio-Ecological Systems | *Current Opinion in Environmental Sustainability* | 2016 |
| [VanHolt_2016_SWIFT...](VanHolt_2016_SWIFT_FisheriesWellbeing_Sustainability.pdf) | A Social Wellbeing in Fisheries Tool (SWIFT) | *Sustainability* | 2016 |
| [VanHolt_2016_Datasets...](VanHolt_2016_Datasets_ScientificInfluence_PLoSONE.pdf) | The Role of Datasets on Scientific Influence within Conflict Research | *PLoS ONE* | 2016 |
| [VanHolt_2017_LandscapeChange...](VanHolt_2017_LandscapeChange_FishingStrategies_STOTEN.pdf) | The Consequences of Landscape Change on Fishing Strategies | *Science of the Total Environment* | 2017 |
| [Caspersen_VanHolt_2017...](Caspersen_VanHolt_2017_ParticipatoryMapping_FieldMethods.pdf) | Measuring Agreement in Participatory Mapping | *Field Methods* | 2017 |
| [Atz_VanHolt_2019_ROSI...](Atz_VanHolt_2019_ROSI_Sustainability_ReviewBusiness.pdf) | The Return on Sustainability Investment (ROSI) | *Review of Business* | 2019 |
| [VanHolt_2020_CulturalConsensus...](VanHolt_2020_CulturalConsensus_SustainabilityInnovation_BSE.pdf) | The Cultural Consensus of Sustainability-Driven Innovation | *Business Strategy & the Environment* | 2020 |

---

## 📬 Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-tracyvanholt-0077B5?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/tracyvanholt)
[![Website](https://img.shields.io/badge/Website-tracyvanholt.com-333333?style=flat-square&logo=google-chrome&logoColor=white)](https://tracyvanholt.com)
[![Email](https://img.shields.io/badge/Email-tvanholt%40gmail.com-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:tvanholt@gmail.com)
